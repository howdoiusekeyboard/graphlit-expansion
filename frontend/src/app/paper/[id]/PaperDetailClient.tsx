...
export default function PaperDetailClient({ paperId }: { paperId: string }) {
  const [minSimilarity, setMinSimilarity] = useState(0.3);

  const { data: paper, isLoading: isPaperLoading } = usePaperDetail(paperId);
  const { data: recData, isLoading: isRecLoading } = usePaperRecommendations(
    paperId,
    12,
    minSimilarity,
  );
